---
title: "Getting Started with overlay"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with overlay}
...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = FALSE # Most examples require an interactive device or external files
)
```
...
**overlay** provides tools to render ggplot2 and gt objects as images, apply perspective distortions, and composite them over background images to create a "Heads-Up Display" (HUD) effect.

## Basic Workflow

The core function is `hud_overlay()`, which handles rendering, optional panelling, warping, and compositing in one go.
...
`hud_panel()` adds a dark frame with an optional border. You can pass a list of arguments to `panel` in `hud_overlay()` to customize this.
...
While `tilt` provides easy presets (`"left"`, `"right"`, `"top"`, `"bottom"`), you can specify `corners` manually for precise control. Each corner is specified as a `c(dx, dy)` offset from its natural position.
